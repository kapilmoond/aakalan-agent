...
app = FastAPI(title="Aakalan Agent", version=__version__, lifespan=_lifespan)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Aakalan Agent (no terminal prompt), "
...
def _count_status_active_sessions() -> int:
    ...
    Aakalan Agent process is using it.
    """
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Aakalan Agent credential-store basenames (see
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Aakalan Agent updates are managed outside this dashboard in "
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Aakalan Agent from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Aakalan Agent to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Aakalan Agent from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Aakalan Agent to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Aakalan Agent in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Aakalan Agent through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Aakalan Agent via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Aakalan Agent to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Aakalan Agent inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Aakalan Agent to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Aakalan Agent through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Aakalan Agent to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Aakalan Agent to Microsoft Teams chats via the Bot Framework.",
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Aakalan Agent.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Aakalan Agent from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Aakalan Agent over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Aakalan Agent through iMessage via Photon's managed Spectrum platform.",
    ...
    "simplex": {
        "description": "Talk to Aakalan Agent over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Aakalan Agent to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Aakalan Agent as an OpenAI-compatible HTTP API for tools like Open WebUI.",
    ...
    "whatsapp_cloud": {
        ...
        "description": "Use Aakalan Agent via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Aakalan Agent Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_whatsapp_onboarding_sessions_loaded = False


def _whatsapp_onboarding_state_file() -> Path:
    from hermes_constants import get_hermes_dir

    return get_hermes_dir("state/whatsapp_onboarding.json", "whatsapp/onboarding_sessions.json")


def _persist_whatsapp_onboarding_sessions() -> None:
    """Persist onboarding sessions so an in-flight pairing survives a gateway restart."""
    try:
        ...
        target = _whatsapp_onboarding_state_file()
        target.parent.mkdir(parents=True, exist_ok=True)
        tmp = target.with_suffix(".tmp")
        tmp.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        tmp.replace(target)
    except Exception:
        _log.exception("Failed to persist WhatsApp onboarding sessions")


def _load_whatsapp_onboarding_sessions() -> None:
    """Restore persisted sessions after a gateway restart.

    Pairing processes do not survive a restart, so any session that was still
    in flight is surfaced as ``expired`` with a clear message instead of a
    confusing 404 ("setup session was not found").
    """
    global _whatsapp_onboarding_sessions_loaded
    if _whatsapp_onboarding_sessions_loaded:
        return
    _whatsapp_onboarding_sessions_loaded = True
    try:
        source = _whatsapp_onboarding_state_file()
        if not source.exists():
            return
        payload = json.loads(source.read_text(encoding="utf-8"))
    except Exception:
        _log.exception("Failed to load WhatsApp onboarding sessions")
        return
    if not isinstance(payload, dict):
        return
    restored = 0
    with _whatsapp_onboarding_lock:
        for pairing_id, data in payload.items():
            ...
            try:
                record = _WhatsAppOnboardingSession(
                    ...
                    allowed_users=str(data.get("allowed_users") or ""),
                ...
                continue
            # The pairing process is gone — only a fresh QR can continue.
            if record.status not in _WHATSAPP_ONBOARDING_TERMINAL_STATUSES:
                record.status = "expired"
                record.error = "WhatsApp setup was interrupted by a gateway restart. Start a new setup."
                record.qr_payload = None
            _whatsapp_onboarding_sessions[pairing_id] = record
            restored += 1
    if restored:
        _log.info("Restored %d WhatsApp onboarding session(s) after gateway restart", restored)
    ...
    try:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            ...
            if session_key and session_key not in joined and "--pair-only" not in joined:
                # Still kill pairing/session processes that mention the session dir.
...
def _whatsapp_bridge_port() -> int:
    ...
    return env_int("WHATSAPP_BRIDGE_PORT", 3000)


async def _whatsapp_session_connected(session_path: Path) -> bool:
    """True only when the on-disk session is real AND a live bridge is connected.

    A bare creds.json is not proof of a working link: baileys writes an empty
    stub the moment a bridge connects (before any scan), and a revoked session
    makes the bridge exit with logged_out. Treating either as "connected" made
    the desktop UI skip the QR code entirely and then fail with
    "WhatsApp setup session was not found. Start a new setup."
    """
    creds_path = session_path / "creds.json"
    try:
        ...
        return False
    me = creds.get("me") if isinstance(creds, dict) else None
    ...
    try:
        ...
        async with aiohttp.ClientSession() as http:
            async with http.get(
                f"http://127.0.0.1:{_whatsapp_bridge_port()}/health",
                timeout=aiohttp.ClientTimeout(total=2),
            ) as resp:
                if resp.status != 200:
                    return False
                data = await resp.json()
                return bool(data and data.get("status") == "connected")
    except Exception:
        return False
...
def _prune_whatsapp_onboarding_sessions() -> None:
    _load_whatsapp_onboarding_sessions()
    now = time.time()
    remove_ids: list[str] = []
    for pairing_id, record in _whatsapp_onboarding_sessions.items():
        if (
...
async def start_whatsapp_onboarding(body: WhatsAppOnboardingStart):
    ...
    with _config_profile_scope(effective_profile):
        ...
        if body.force_new:
            # Connecting a different phone must never reuse leftover creds.json
            # from the previous number.
            _wipe_whatsapp_identity()
            session_path = _whatsapp_session_path()
        if not body.force_new and await _whatsapp_session_connected(session_path):
...
async def apply_whatsapp_onboarding(
    ...
    try:
        with _config_profile_scope(effective_profile):
            ...
            # Always write the allowlist for this phone. Keeping the previous
            # number is why a second phone shows "connected" but never replies.
...
async def disconnect_whatsapp(profile: Optional[str] = None):
    ...
    try:
        with _config_profile_scope(effective_profile):
            # Stop the live bridge first. On Windows an open Node process
            # locks creds.json, so rmtree alone leaves the old phone behind.
            _wipe_whatsapp_identity()
            save_env_value("WHATSAPP_ENABLED", "false")
            _write_platform_enabled("whatsapp", False)
            _wipe_whatsapp_identity()
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Aakalan Agent").strip() or "Aakalan Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Aakalan Agent appears
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Aakalan Agent even
    when they also have a separate Aakalan Agent-managed PKCE login.
    """
    try:
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Aakalan Agent, so the disconnect
    ...
    user sees exactly what executes, and Aakalan Agent then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Aakalan Agent — run the disconnect command to remove it."
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Aakalan Agent file AND credential pool.
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Aakalan Agent "
...
_SKILL_HUB_SOURCE_LABELS = {
    ...
    "hermes-index": "Aakalan Agent Index",
    "skills-sh": "skills.sh",
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Aakalan Agent Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Aakalan Agent inside WSL2 to use the dashboard's /chat "
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Aakalan Agent Teal canvas before the SPA flips the configured user
    theme into place.
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Aakalan Agent Teal",         "description": "Classic dark teal — the canonical Aakalan Agent look"},
    {"name": "default-large", "label": "Aakalan Agent Teal (Large)", "description": "Aakalan Agent Teal with bigger fonts and roomier spacing"},
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Aakalan Agent backend listening on {host}:{actual_port}")
            else:
                print(f"  Aakalan Agent Web UI → http://{host}:{actual_port}")
