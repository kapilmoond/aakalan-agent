...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Aakalan Agent'
...
let bootProgressState = {
  ...
  message: 'Waiting to start Aakalan Agent backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Aakalan Agent will start automatically when it completes…',
        12
      )
    },
  ...
  try {
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Aakalan Agent update did not finish',
...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    throw new Error(`Could not persist ownership for the Aakalan Agent backend: ${error.message}`)
  }
}
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Aakalan Agent — this window will close and the updater will open. Don’t reopen Aakalan Agent yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    if (!lock.unlocked) {
      ...
      const message =
        'Update aborted: another process is holding the Aakalan Agent install open ' +
        '(a second Aakalan Agent window or a terminal running hermes?). Close it and retry.'
    ...
    if (scriptHandoff) {
      ...
      // abort with "Another Aakalan Agent update is already running (PID <itself>)" —
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Aakalan Agent — this window will close. Don’t reopen Aakalan Agent yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Aakalan Agent.app/Contents/...`
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Aakalan Agent at ${ACTIVE_HERMES_ROOT}`,
    command,
    args: ['-m', 'hermes_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: HERMES_HOME,
...
function resolveHermesBackend(backendArgs) {
  ...
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Aakalan Agent source at ${overrideRoot}`, backendArgs)
  ...
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Aakalan Agent source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Aakalan Agent runtime at ${ACTIVE_HERMES_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Aakalan Agent CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (shouldTrustHermesOverride(hermesOverride) || verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        ...
        return {
          label: `existing Aakalan Agent CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Aakalan Agent CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Aakalan Agent install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Aakalan Agent recovery was handed off to Aakalan Agent Setup. The desktop will restart when recovery completes.'
      )
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Aakalan Agent install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Aakalan Agent bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (!isHermesSourceRoot(ACTIVE_HERMES_ROOT)) {
    throw new Error(
      `Aakalan Agent install at ${ACTIVE_HERMES_ROOT} is missing or incomplete. ` +
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Aakalan Agent on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Aakalan Agent.'
    )
  }
  ...
  backend.label = `Aakalan Agent at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Aakalan Agent runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Aakalan Agent backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Aakalan Agent backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Aakalan Agent backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Aakalan-Agent-Session-Token`` header — used by the auth-mode probe before
...
// signal so the chat composer doesn't stay stuck on "Starting Aakalan Agent...".
...
// single-shot boot() with no retry) surfaced as the "Aakalan Agent couldn't start"
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Aakalan Agent Cloud agent…' : 'Sign in to Aakalan Agent gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Aakalan Agent backend after ${timeoutMs}ms`))
    }, timeoutMs)
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Aakalan Agent Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Aakalan Agent Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Aakalan Agent Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Aakalan Agent Cloud. Open Settings → Gateway, choose Aakalan Agent Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Aakalan Agent Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Aakalan Agent Cloud session has expired. Sign in to Aakalan Agent Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    if (
      ...
      const err = new Error(
        'Remote Aakalan Agent gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Aakalan Agent gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Aakalan Agent gateway is selected, but no session token is saved. ' +
    ...
    if (!rawEnvToken) {
      throw new Error(
        'HERMES_DESKTOP_REMOTE_URL is set but HERMES_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Aakalan Agent backend.'
      )
    }
        ...
        try {
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Aakalan Agent on the remote host before connecting with Desktop SSH.'
            }
          }
...
async function spawnPoolBackend(profile, entry) {
  ...
  rememberLog(`Starting Aakalan Agent backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Aakalan Agent backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Aakalan Agent backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Aakalan Agent backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Aakalan Agent backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Aakalan Agent backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
async function startHermes() {
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Aakalan Agent backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote Aakalan Agent backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Aakalan Agent backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Aakalan Agent runtime', 28)
    ...
    await advanceBootProgress('backend.spawn', `Starting Aakalan Agent backend via ${backend.label}`, 84)
    rememberLog(`Starting Aakalan Agent backend via ${backend.label}`)
    ...
    if (!processOwner) {
      ...
      throw new Error('Aakalan Agent backend start was superseded by a newer connection attempt.')
    }
    ...
    hermesProcess.once('error', error => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Aakalan Agent backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Aakalan Agent backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Aakalan Agent backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Aakalan Agent backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Aakalan Agent backend exit (${signal || code})`)
      ...
      rememberLog(`Aakalan Agent backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Aakalan Agent backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Aakalan Agent backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Aakalan Agent backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Aakalan Agent backend to become ready', 90)
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Aakalan Agent backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Aakalan Agent backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// "Starting Aakalan Agent…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    if (!backend.command) {
      return { ok: false, error: 'Aakalan Agent is not installed yet' }
    }
...
ipcMain.handle('hermes:notify', (_event, payload) => {
  ...
  const notification = new Notification({
    title: payload?.title || 'Aakalan Agent',
    body: payload?.body || '',
...
function terminalShellEnv() {
  ...
  env.TERM_PROGRAM = 'Aakalan Agent'
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Aakalan Agent agent venv at ${VENV_ROOT}.`
    }
  }
